...
export default function EditProductPage() {
  ...
  const { isAdmin, adminProducts, updateProduct } = useAdmin()
  ...
  useEffect(() => {
    ...
    const loadProduct = async () => {
      ...
      if (product) {
        ...
        setStockOut(product.inStock === false)

        const initialSelected: Record<string, boolean> = Object.fromEntries(
          PERFUME_SIZE_OPTIONS.map((s) => [s, false])
        )
        const initialPrices: Record<string, string> = Object.fromEntries(
          PERFUME_SIZE_OPTIONS.map((s) => [s, ''])
        )

        const variantPrices = product.variantPrices
        if (variantPrices && typeof variantPrices === 'object') {
          for (const size of Object.keys(variantPrices)) {
            if (size in initialSelected) {
              initialSelected[size] = true
              initialPrices[size] = String(variantPrices[size])
            }
          }
          ...
          for (const size of product.sizes) {
            if (size in initialSelected) {
              initialSelected[size] = true
              initialPrices[size] = String(product.price.min ?? '')
            }
          }
        }

        setSelectedSizes(initialSelected)
        setPricesBySize(initialPrices)
        setLoading(false)
        return
      }
        ...
        if (response.ok) {
          ...
          if (data.products && data.products.length > 0) {
            ...
            setFormData({
              ...
              notes: foundProduct.notes?.join(', ') || '',
              isHot: foundProduct.isHot,
            })

            setImageDataUrl(foundProduct.image || '')
            setStockOut(foundProduct.inStock === false)
            ...
            const variantPrices = foundProduct.variantPrices
            if (variantPrices && typeof variantPrices === 'object') {
              for (const size of Object.keys(variantPrices)) {
                if (size in initialSelected) {
                  initialSelected[size] = true
              ...
              for (const size of foundProduct.sizes) {
                if (size in initialSelected) {
                  initialSelected[size] = true
                  initialPrices[size] = String(foundProduct.price.min ?? '')
                }
              }
            }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert('Product updated successfully and synced to database!')
      ...
      <div className="bg-gradient-to-r from-burgundy-900 via-burgundy-800 to-burgundy-900 text-white py-6 shadow-md">
        <div className="container-fluid">
          <div className="flex items-center space-x-4">
            ...
            <div>
              <h1 className="text-3xl font-bold">Edit Perfume</h1>
              <p className="text-burgundy-100">Update product details</p>
            </div>
          </div>
        </div>
      </div>
        ...
        <div className="bg-white rounded-lg shadow p-8 max-w-2xl">
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex space-x-4 pt-6">
              ...
              >
                {isSubmitting ? 'Updating...' : 'Update Perfume'}
              </button>
              <Link
